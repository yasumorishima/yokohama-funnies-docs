...
| Metric | Count |
|--------|-------|
| TypeScript/TSX files | <!--stat:ts_files-->165<!--/stat--> |
| Lines of code | <!--stat:loc-->~23100<!--/stat--> |
| Page routes | <!--stat:pages-->48<!--/stat--> |
| API routes | <!--stat:apis-->10<!--/stat--> |
| Reusable components | <!--stat:components-->61<!--/stat--> |
| DB tables (+ history) | <!--stat:tables_main-->26<!--/stat--> + <!--stat:tables_hist-->5<!--/stat--> |
| DB migrations | <!--stat:migrations-->60<!--/stat--> |
...
| `schedule_news` | Anon teaser of upcoming games (notes/created_by 除外, home 新着情報用) |
| `results_news` | Anon teaser of game results (note/inning_scores 除外, home 新着情報用) |
...
| **Dependency vulnerability management** | 2026-06-10 全数監査 (姉妹サイトと同時): 本番依存パッケージを OSV.dev (Google 脆弱性DB) で照会し、next 15.5.14 の既知脆弱性 14 件 (HIGH: middleware/proxy バイパス GHSA-267c-6grr-h53f / GHSA-26hh-7cqf-hhc6 / GHSA-492v-c6pp-mqqv、DoS、SSRF ほか) を検出 → 即日 next 15.5.18 / ws 8.21.0 へ bump。本サイトは認可を middleware で行うためバイパス系は直撃構成だった (データ自体は RLS の defense-in-depth で保護)。2026-07-05、残存の moderate 3 件 (postcss XSS・js-yaml DoS・brace-expansion DoS。いずれもビルド/開発時のみで実行時露出なし) を `package.json` の `overrides` で patched 版に固定して解消 (`npm audit` = 0 / Dependabot open = 0)。2026-07-27、high 4 系統を解消: **sharp 0.35.3** (GHSA-f88m-g3jw-g9cj = libvips CVE-2026-33327/33328/35590/35591。直接依存を上げるだけでは不足で、next が同梱する nested sharp 0.34.5 も `overrides` で巻き取る必要があった)、**next 15.5.22** (SSRF・DoS・cache confusion 等 8 勧告が <15.5.21 対象。16 系はメジャーのため据え置き)、**postcss 8.5.23** (2026-07-05 に固定した 8.5.16 が新勧告 <=8.5.17 の範囲に入っていた＝過去に潰した override も再検証が要る)、**brace-expansion 1.1.16**。`npm audit --omit=dev` = 0 / Dependabot open = 0、Vercel deploy も success。残る brace-expansion の勧告 (範囲 <=5.0.7 で 1.x 系も semver 上は該当) は lint 専用で本番バンドルに載らず、eslint-plugin-import / react / jsx-a11y が最新版でも minimatch@^3 依存のため上流に修正版が存在しない (minimatch を上げると CJS export が関数でなくなり plugin が壊れる)。glob パターンは自前の eslint 設定由来で攻撃者の制御下にないため tolerable_risk として dismiss、上流が minimatch を上げたら再評価 |
...
Hero → トップ通知バー (最新の新着情報1件、押下で #news へジャンプ) → CONTENTS ナビバンド (5 色 card-style + 横スライド snap) → 横浜スタジアム 3 日天気 (M/D + 曜日 + 降水% + 累計 N.Nmm + 雨強度別アイコン、クリックで /weather に遷移) → チームの一言 → № 01 ABOUT US → № 02 MOMENTS (写真) → № 03 NEWS (お知らせ + 監督メッセージ + オーダー表 + 試合予定 + 試合結果を *_news view で合流。お知らせ行は複製しない) → № 04 UP NEXT (Google Calendar 風 月間ビュー) → № 05 RECORD → № 06 ROSTER (公開選手名簿)
